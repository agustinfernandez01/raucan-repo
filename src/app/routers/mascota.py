...
def crear_mascota(datos: MascotaCreate, db: Session = Depends(get_db)):
    ...
    return svc_mascota.crear(db, datos)
...
def actualizar_mascota(
    ...
    mascota = svc_mascota.actualizar(db, mascota_id, datos)
    if mascota is None:
        raise HTTPException(status_code=404, detail="Mascota no encontrada")
    return mascota
...
def eliminar_mascota(mascota_id: int, db: Session = Depends(get_db)):
    """Elimina una mascota."""
    if not svc_mascota.eliminar(db, mascota_id):
        raise HTTPException(status_code=404, detail="Mascota no encontrada")
    return None
